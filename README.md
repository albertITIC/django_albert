# django_albert
## Punt 1
## Punt 1.1
### Informació alumnat
![Info alumnat.png](image.png)

### Informació professorat
![Info professorat.png](image-1.png)

## Punt 1.2 (millorat i amb header)
### Informació alumnat
![Html millorat + Header afegit.png](image-4.png)

### Informació professorat
![Html millorat + Header afegit.png](image-2.png)

### Distribució del projecte i de l'app
![Projcte de tota l'activitat 13 (fins on tinc fet).png](img.png)

## Punt 2

## Punt 3

##### ---------------------------------------------------------------------------------------------------------------------------
# Activitat 14
#### Inici de la pràctica 12-02-25
## Pre-activitat
#### · He cambiat la versió de python: per la 3.10.0
![Versió de python.png](img_1.png)

#### · Configuració Pycharm (interpreter per python i la nova versió).
![Interpreter.png](img_7.png)

#### · Aplicar canvis i els guardo.
![Aplico canvis.png](img_8.png)

#### · Resulat final:
![img_9.png](img_9.png)

###  · Creació de l'entorn virtual
![Creació del meu seon entorn virtual.png](img_2.png)

###  · Activació de l'entorn
![Activació del segon entorn virtual.png](img_3.png)

### ·  Instal·lació del Django (dins del nou entorn virtual)
![Instal·lació Django.png](img_4.png)

###  · Comprobació de la versió Django sigui la correcte
![Versió Django.png](img_5.png)

###  · Instal·lació de pyscopg2 per PostgresSQL
![Instalació de psycopg2.png](img_6.png)

### · La creació del  projecte (seguiment de l'anterior)
#### En aquest projecte seguiré amb el projecte creat anteriorment, el directori: TIC_BCN_APC 
Aquest seria el resultat de la distribució de l'activitat 14:
![Directoris.png](img_10.png)

### · Creació del fitxer Docker (docker.compose.yml)
#### Prèviament comprovo si ho tinc instal·lat
